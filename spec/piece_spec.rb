...
describe 'valid moves' do
  describe 'king' do
    ...
    it 'allowed moves' do
      # Horizontal move
      expect(king.valid_moves([0, 4], [0, 5])).to be true

      # Vertical move
      expect(king.valid_moves([0, 4], [1, 4])).to be true

      # Diagonal move
      expect(king.valid_moves([0, 4], [1, 3])).to be true
    end
  ...
  describe 'pawn' do
    ...
    it 'allowed moves' do
      # One step ahead
      expect(pawn_black.valid_moves([1, 3], [2, 3])).to be true
      expect(pawn_white.valid_moves([6, 3], [5, 3])).to be true

      # Two steps if first move
      expect(pawn_black.valid_moves([1, 1], [3, 1])).to be true
      expect(pawn_white.valid_moves([6, 3], [4, 3])).to be true
    end
